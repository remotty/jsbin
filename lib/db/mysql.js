...
module.exports = utils.inherit(Object, {
  ...
  constructor: function MySQL(options) {
    ...
    this.defaults   = {html: '', css: '', javascript: '', spec: '', dataset: ''};
  ...
  setBinOwner: function (params, fn) {
    var sql = templates.setBinOwner,
        values = [params.name, params.url, params.revision, new Date(), params.summary, params.html, params.css, params.javascript, params.spec, params.dataset, params.visibility || 'public'];
  ...
  setBinPanel: function (panel, params, fn) {
    ...
    allowed = {html: 1, css: 1, javascript: 1, spec: 1, dataset: 1},
  ...
  populateOwners: function (params, fn) {
    // params.date is only for use when populating the summary field
    var values = [params.date || new Date(), params.summary, params.html, params.css, params.javascript, params.spec, params.dataset, params.name, params.url, params.revision];
  ...
  binExistsByName: function(name, fn){
    this.connection.query(templates.binExists, [name], function (err, results) {
      if (err) {
        fn(err);
      }else{
        fn(null, results.length > 0 ? true : false);
      }
    });
  },

  removeBin: function(bin_url, params, fn){
    var error = false;
    
    this.connection.query(templates.removeOwner, [bin_url], function (err, results) {
      if(err){
        fn(err);
      }
    });
    
    this.connection.query(templates.removeBin, [bin_url], function (err, results) {
      fn(err);
    });
  },

  renameBin: function(names, params, fn){
    var error = false;
    var names_array = [names.new_name, names.old_name]
    
    this.connection.query(templates.renameOwner, names_array, function (err, results) {
      if(err){
        fn(err);
      }
    });
    
    this.connection.query(templates.renameBin, names_array, function (err, results) {
      fn(err);
    });
  },

  // for tests
  removeAllBins: function (params, fn) {
    var query = "delete from sandbox;";
