...
module.exports = Observable.extend({
  ...
  createBin: function (req, res, next) {
    var bin = utils.extract(req.body, 'html', 'css', 'javascript', 'settings', 'url');
    ...
    this.validateBin(params, function (err) {
      ...
      _this.models.bin.create_with_name(bin, function (err, result) {
        if (err) {
          return next(err);
        }
  ...
  validateBin: function (bin, fn) {
    ...
    var panels = 'html javascript css spec dataset'.split(' ').map(function (key) {
      return (bin[key] || '').trim();
          ...
          function (err, html) {
            if (req.ajax) {
              ...
              res.render('list', { list_history: html,
                jsbin: jsbin,
                static: helpers.urlForStatic(),
                scripts: [ '/vendor/js/libraries/jquery-1.11.0.min.js',
                  '/vendor/js/libraries/pretty-date.js',
  ...
  jsbin: function (bin, options) {
    ...
    // jasmine -> spec, dataframe -> dataset
    var panels = binSettings.panels || Object.keys(_.pick(bin, 'html', 'javascript', 'css', 'jasmine', 'dataframe')).filter(function (panel) {
  ...
  templateFromBin: function (bin) {
    var template = utils.extract(bin, 'html', 'css', 'javascript', 'spec', 'dataset');

    'html css javascript spec dataset'.split(' ').forEach(function (panel) {
    ...
    template.dataframe = template.dataset;
    template.jasmine = template.spec;
    ...
    this.loadFiles(null, function (err, defaults) {
      ...
      bins.forEach(function (bin) {
        ...
        revisions.push({
          ...
          is_private: bin.visibility === 'private' ? true : false,
  ...
  renameBin: function (req, res, next) {
    // This is duplication of delete
    ...
    var names = {
      old_name: req.bin.url,
      new_name: req.body.new_name
    }
    
    this.models.bin.renameBin(names, {}, function(error){
      if (error) {
        console.error(error);
  ...
  deleteAll: function (req, res, next) {
    ...
    this.models.bin.removeBin(req.bin, {}, function(error){
      if (error) {
        console.error(error);
        return res.send(400, {error: 'The bin could not be deleted.', detail: error });
      }
