...
var github      = require('./github')(options); // if used, contains github.id
var dropbox     = require('./dropbox')(options);
var _           = require('underscore');
var crypto      = require('crypto');
var filteredCookieSession = require('express-cookie-blacklist');
var stripeRoutes  = require('./stripe')(options);
var flattened;
...
app.mailer = (function (mail) {
  ...
  if (method && options) {
    mailTransport = require('nodemailer').createTransport(method, settings);
  }

  return new require('./handlers').MailHandler(mailTransport, app.render.bind(app));
})(options.mail);
...
app.connect = function (callback) {
  ...
  store.connect(function (err) {
    ...
    // finally run the sync methods in the model creation
    require('./models').createModels(store);
