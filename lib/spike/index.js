...
utils.process = function(data, settings) {
  return new Promise(function (resolve, reject) {
    ...
    if (!processorSettings) {
      return resolve({ result: data.content });
    }
    ...
    // only process if we're CSS, since the spike forces a reload on HTML & JS
    if (processors.lookup[processorName] !== 'css') {
      return resolve({ result: data.content })
    }

    return processors.run(processorName, {source: data.content}).then(resolve);
  });
};
...
var spike = module.exports = {
   ...
   */
  ping: function (bin, sourceData, statsRequest) {
    var id = bin.id,
        delayTrigger = 500;
...
{
  bin: <bin object - unprocessed>
  timer: <result of setTimeout>
  html: { // can be css, javascript
    content: <string - post processor>
...
*/
      ...
      utils.process(sourceData, bin.settings).then(function (content) {
        if (content.errors) {
console.log('processor had errors', content.errors);
          return;
        }

        pending[id][sourceData.panelId] = {
  ...
  console.log('data?, session?', !!data, !!session);
        ...
        if (ping.css && ping.css !== content.result) {
          data.raw = bin.css;
        }
        ...
        session.res.forEach(function (res) {
          ...
          if (!res.req.stats) {
            console.log('writing', sourceData.panelId, raw)
            res.write(utils.makeEvent(sourceData.panelId, raw)); // for codecasting
            res.write(utils.makeEvent(sourceData.panelId + ':processed', sourceData.panelId === 'css' ? data.content : '    ')); // for live reload
          }
