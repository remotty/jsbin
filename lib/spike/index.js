...
utils.process = function(data, settings) {
  return new Promise(function (resolve, reject) {
    ...
    return processors.run(processorName, {source: data.content});
  });
};
  ...
  ping: function (bin, sourceData, statsRequest) {
    ...
    pending[id].timer = setTimeout(function () {
      ...
      utils.process(sourceData, bin.settings).then(function (content) {
        ...
        pending[id][sourceData.panelId] = content.result;
        ...
        session.res.forEach(function (res) {
          ...
          if (!res.req.stats) {
            console.log('writing', data.panelId, raw)
