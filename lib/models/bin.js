...
var binPanels = ['html', 'css', 'javascript', 'spec', 'dataset'];
...
var model = {
  ...
  // Create a new bin with specific name
  create_with_name: function (data, fn) {
    var id = data.url
    data.revision = 1;
    data.latest = true;
    data.streamingKey = this.createStreamingKey(id, data.revision);

    this.store.binExistsByName(id, function(err, is_exists) {
      if (err) {
        return fn(err);
      }

      if(is_exists){
        return fn("This name already taken.")
      }
  ...
  removeBin: function(bin, params, fn){
    this.store.removeBin(bin.url, params, fn);
  },
  renameBin: function(names, params, fn){
    this.store.renameBin(names, params, fn);
  }
};
